...
<body>
	...
	<%--
	 ...
	 %>
	 <c:forEach var="season" items="${seasons}" varStatus="vs">		<%-- 인덱스 값을 쓰고 싶다면 varStatus 속성을 쓰면 된다  --%>
	 	<div>인덱스: ${vs.index}, 순번: ${vs.count}, 요소: ${season}</div>
	</c:forEach>
	 
	 <%-- 리스트 --%>
	 <%
	 	List<String> jobs = Arrays.asList("개발자", "기획자", "디자이너", "퍼블리셔");
	 ...
	 <%
	 	...
	 	webtoonList.add(new Webtoon(419, "HBLJ", 9.98, LocalDate.of(2022, 12, 14)));
