...
<body>
   ...
   <%--
	...
	<%
		...
		book.put("author", "양귀자");
		book.put("title", "나는 희망한다 내게 금지된 것을");
		book.put("price", 24000);
	...
	<%-- 객체 사용하기 --%>
     <%
     	...
     	pageContext.setAttribute("boook2", book2);
     %>
     <ul>
     	<li>저자: ${book2.author}</li>		<%-- 자동으로 ${book2.getAuthor()}가호출되어 실행된다. --%>
     	<li>제목: ${book2.title}</li>		<%-- 자동으로 ${book2.getTitle()}가호출되어 실행된다. --%>
		<li>가격: ${book2.price}</li>     	<%-- 자동으로 ${book2.getPrice()}가호출되어 실행된다. --%>
		<%-- 자동으로 getter가 호출된다. 클래스 만들 때 getter&setter 만들어두기 --%>
     </ul>
